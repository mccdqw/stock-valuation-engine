...
def simple_market_fill(df: pl.DataFrame, initial_capital=10000, return_trades=False):
    ...
    if "date" in df.columns:
        timestamps = df.get_column("date").to_list()
    else:
        timestamps = list(range(df.height))

    equity_dict = dict(zip(timestamps, df.get_column("equity_curve").to_list()))
    returns_dict = dict(zip(timestamps, df.get_column("strategy_returns").to_list()))

    trades = []
    if return_trades:
        signal_list = df.get_column("signals").to_list()
        close_list = df.get_column("close").to_list()
        prev_signal = 0
